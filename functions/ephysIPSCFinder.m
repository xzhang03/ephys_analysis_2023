function fn_out = ephysIPSCFinder(fpath, varargin)
...
addOptional(p, 'defaultprom', 14); % Default prominence threshold
addOptional(p, 'defaultcurrent', 7); % Default current threshold
addOptional(p, 'defaultwidth', 100); % Default wid threshold
...
xlabel('Time (s)')
ylabel('Current (pA)')
title(sprintf('IPSC detection: %s', fn));
