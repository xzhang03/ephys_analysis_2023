function fn_out = ephysEPSCFinder(fpath, varargin)
...
addOptional(p, 'movmean', false);
addOptional(p, 'movmeanwin', 3);
...
addOptional(p, 'defaultprom', 6); % Default prominence threshold
addOptional(p, 'defaultcurrent', 4); % Default current threshold
addOptional(p, 'defaultwidth', 20); % Default wid threshold
...
addOptional(p, 'spikewindow', [-200 200]); % Window to display spikes, in ms
...
xlabel('Time (s)')
ylabel('Current (pA)')
title(sprintf('EPSC detection: %s', fn));
...
function promchange(src, ~)
    ...
    % Update means
    indsmat = ones(length(locs),1) * inds + locs * ones(1, length(inds));
    ikeep = (indsmat(:,end) <= l) & (indsmat(:,1) >= 1); 
    indsmat = indsmat(ikeep, :);
...
function savedata(src, ~)
    ...
    params = struct('locs', locs, 't0', t0, 'tmax', tmax, 'fs', fs, 'promt',...
        ...
        proms, 'dataparams', dataparams, 'finderparams', p, 'ws', ws); % 'wvs', wvs
    else
        params.locs = locs;
        params.proms = proms;
        params.promt = promt;
